...
left_join(subset(res, pop != "EUR", select=c(rsidcode, maf, pop)), subset(res, pop == "EUR", select=c(rsidcode, maf)), by="rsidcode") %>%
  ...
  group_by(pop) %>%
  do({
    summary(lm(gvar.x ~ gvar.y, .))$coef %>% as_tibble() %>% slice(n=2)
  })
```
...
```{r}
ratios %>%
  filter(rsq_ratio > quantile(rsq_ratio, 0.05) & rsq_ratio < quantile(rsq_ratio, 0.95)) %>%
...
```{r}
group_by(temp, pop) %>%
  do({summary(lm(I(1/Fstat.x) ~ I(1/Fstat.y), data=.))$coef %>% as_tibble %>% slice(n=2)})
```
