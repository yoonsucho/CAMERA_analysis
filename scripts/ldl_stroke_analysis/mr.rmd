...
lapply(1:5, \(i) {l[[i]] %>% mutate(outcome=stroke_traits[i])}) %>% bind_rows() %>% {
    ggplot(., aes(x=Estimate, y=pops)) +
    geom_point(data=. %>% filter(pops != "All"), aes(colour=pops, size=-log10(Qjpval))) +
...
}) %>% bind_rows() %>% mutate(outcome=stroke_traits)
```
...
inner_join(subset(temp, datum=="Expected"), subset(temp, datum == "Observed"), by=c("disc", "rep", "outcome")) %>%
    ...
    tidyr::pivot_wider(names_from=c(datum, disc, rep, outcome), values_from=value)
```
